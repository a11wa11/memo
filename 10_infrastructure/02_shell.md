...
# umask(ユーマスク)
umask    # 現在のumaskを確認             0022
umask -S # 現在のumaskをシンボリック表示確認 u=rwx,g=rx,o=rx

umask 0022             # 新規作成ファイルのデフォルト権限を644にする
umask 0002             # 新規作成ファイルのデフォルト権限を664にする
umask u=rwx,g=rwx,o=rx # 新規作成ファイルのデフォルト権限を775にする
```
...
```sh
# tar.gzファイルでカレントディレクトリに圧縮
tar -czvf ファイル名．tar.gz 対象ディレクトリ
# カレントディレクトリから対象ディレクトリを指定する方が望ましい
# tar -cxvf ファイル名.tar.gz ディレクトリ1/ディレクトリ2/対象ディレクトリ
# ↑で圧縮したファイルだと対象ディレクトリにあるファイルが圧縮されるが、ディレクトリ1/ディレクトリ2の構造も含まれてしまう

# zipファイルでカレントディレクトリに圧縮
zip 作成したいファイル名.zip 対象ファイル
zip 作成したいファイル名.zip -r 対象ディレクトリ

# gzファイルでカレントディレクトリに圧縮
gzip ファイル名
```
...
```sh
# カレントディレクトリに解凍
tar -xzvf ファイル名.tar.gz # オプションのxがExtract(展開)の意味

# 指定ディレクトリに解凍
tar -xvzf ファイル名.tar.gz -C ディレクトリ名
```

- zip形式

```sh
# zipファイルでカレントディレクトリに解凍
unzip 対象ファイル
```
- gz形式

```sh
# gzファイルでカレントディレクトリに解凍
gunzip 対象ファイル
```
...
- eval
  - 引数として与えられたコマンドを再度評価(evaluate)して実行する
  - ex. `eval echo '$'$ENV`
...
# -0 pipefail パイプラインの終了ステータスを「最後のコマンド」ではなく、「失敗したコマンドがあれば失敗」にする。例: cmd1 | cmd2 で cmd1 が失敗しても cmd2 が成功すると通常は成功扱いだが、pipefail があると失敗扱いになる
set -euo pipefail
```
