...
ヘッダーの役割はボディ(実データ)の説明書

- レスポンスヘッダー: サーバーがブラウザに返す際に送信するヘッダーの総称
   - セキュリティヘッダー: レスポンスヘッダーの中で、ブラウザレベルのセキュリティ機能を有効化するもの

| ヘッダー種別 | 項目名 | 役割 | 補足 |
| - | - | - | - |
| レスポンスヘッダー | Content-Type | ブラウザが何をすべきか判断 |  |
| レスポンスヘッダー | Cache-Control | キャッシュをどうするか指示 |  |
| レスポンスヘッダー | Set-Cookie | ブラウザにクッキーを保存させる |  |
| レスポンスヘッダー | Security-Header | ブラウザのセキュリティ動作を制御 | ※次のレイヤー |
| セキュリティヘッダー | Strict-Transport-Security | HTTPS強制 |  |
| セキュリティヘッダー | X-Content-Type-Options | MIME型スニッフィング防止 |  |
| セキュリティヘッダー | X-Frame-Options | クリックジャッキング対策 |  |
| セキュリティヘッダー | Content-Security-Policy | XSS対策 |  |
| セキュリティヘッダー | Referrer-Policy | 参照元情報制御 |  |
